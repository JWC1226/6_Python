"""
    고객(Customer) 도메인 모델과 커스텀 예외 클래스(DuplicateContactError, CustomerNotFoundError)를 정의합니다
    클래스 계층은 Customer(부모) ⇒ NormalCustomer, VIPCustomer(자식)입니다
"""
# 1. 커스텀 예외 클래스
class DuplicateContactError(Exception):
    # 이미 등록된 연락처로 신규 등록/수정을 시도할 때 발생시키는 예외
    pass

class CustomerNotFoundError(Exception):
    # 존재하지 않는 고객 ID로 조회/수정/삭제를 시도할 때 발생시키는 예외
    pass

# 2. 고객 모델 클래스 (부모 클래스)
class Customer:
    # 모든 고객 유형이 공통으로 가지는 속성과 동작을 정의하는 부모 클래스
    # NormalCustomer, VIPCustomer는 이 클래스를 상속받아 세부 동작(포인트 적립률 등)만 다르게 구현한다
    ...
    def add_purchase(self, amount):
        # 구매 금액(amount)을 입력받아 포인트를 적립하는 메서드
        # 실제 적립률은 등급별로 다르므로 자식 클래스에서 "반드시" 오버라이딩
        raise NotImplementedError("자식 클래스에서 add_purchase()를 구현해야 합니다")

    def is_vip(self):
        # VIP 여부를 반환합니다
        # 기본 Customer는 False이며 VIPCustomer에서 True로 오버라이딩
        return False

    def __str__(self):
        # 모든 고객 클래스가 공통으로 사용할 기본 출력 (format)
        return f"[ID:{self.customer_id}] {self.name} | 연락처: {self.contact} | 누적 포인트: {self.points}점"

# 3. 일반 고객 클래스 (자식 클래스)
class NormalCustomer(Customer):
    # 일반 고객: 구매 금액의 1%를 포인트로 적립됩니다
    ...
    def __str__(self):
        # 부모의 공통 포맷을 재사용하고, 등급 표시만 추가
        return f"{super().__str__()} | 등급: 일반"

# 4. VIP 고객 클래스 (자식 클래스)
class VIPCustomer(Customer):
    # VIP 고객: 구매 금액의 5%를 포인트로 적립하며, 전담 매니저 이름을 가질 수 있다
