"""
CRMManager 클래스가 고객 데이터를 메모리 리스트에 저장하고 등록/조회/수정/구매/탈퇴 로직을 처리합니다
데이터는 self.customers에만 보관되며 프로그램 종료 시 사라집니다
"""
import re
...
class CRMManager:
    ...
    def _is_duplicate_contact(self, contact, exclude_id=None):
        # 주어진 연락처가 이미 등록되어 있는지 확인
            # exclude_id를 지정하면 해당 ID의 고객은 검사 대상에서 제외
            # (연락처 수정 시 자기 자신 제외용)
    ...
    def _validate_contact_format(self, contact):
        # 연락처가 '0xx-xxx(x)-xxxx' 형식의 전화번호인지 검사 / 아니면 ValueError를 발생시킨다
    ...
    def register_customer(self, customer):
        # 고객 객체를 등록하고 등록된 객체를 반환한다
            # 연락처 형식이 잘못되면 ValueError
            # 이미 등록된 연락처면 DuplicateContactError를 발생
    ...
    def find_by_min_points(self, min_points):
        # 누적 포인트가 min_points 이상인 고객을 검색하여 반환한다
            # min_points가 음수이면 ValueError를 발생
    ...
    def update_contact(self, customer_id, new_contact):
        # ID로 고객을 찾아 연락처를 수정한다
            # 고객이 없으면 CustomerNotFoundError,
            # 형식이 잘못되면 ValueError,
            # 다른 고객과 중복되면 DuplicateContactError를 발생
    ...
    def change_grade(self, customer_id, new_grade, manager_name=None):
        # ID/이름/연락처/포인트를 유지한 채 NormalCustomer<->VIPCustomer 객체로 교체하여 등급을 전환
            # 고객이 없으면 CustomerNotFoundError,
            # new_grade가 올바르지 않거나 이미 같은 등급이면 ValueError를 발생
        customer = self._find_by_id(customer_id)
    ...
    def update_manager(self, customer_id, new_manager_name):
        # VIP 고객의 담당 매니저 이름을 변경한다
            # 고객이 없으면 CustomerNotFoundError,
            # VIP 고객이 아니면 ValueError를 발생
        customer = self._find_by_id(customer_id)
    ...
    def purchase(self, customer_id, amount):
        # ID로 고객을 찾아 구매 금액만큼 포인트를 적립하고 적립된 포인트를 반환한다
            # 고객이 없으면 CustomerNotFoundError,
            # 금액이 음수면 ValueError를 발생
        customer = self._find_by_id(customer_id)
    ...
    def withdraw_customer(self, customer_id):
        # ID로 고객을 찾아 리스트에서 삭제한다
        # 고객이 없으면 CustomerNotFoundError가 발생
