...
# 상관 히트맵: 여러 변수들 간의 상관 관계를 계산한 데이터(표)를 색상의 농도와 밝기로 표현한 그래프
pivot = df.pivot_table(index="date", columns="code", values="ret")
    # 긴 형식(한 행=한 종목의 하루 데이터) > 넓은 형식(행=날짜, 열=종목)
    # 섹터순으로 열을 정렬해야 블록이 제대로 표시될 것이다 !
order = df[["code", "sector"]].drop_duplicates().sort_values(["sector", "code"])["code"].tolist()
pivot = pivot[order]    # 열 순서를 정렬했으므로、데이터는 그대로고 순서만 바뀔 것이다 !
# corr() : 열끼리 상관 계수 행렬 > (120, 120)
    # -1(정반대) ~0(무관) ~1(동일하게 움직인다) / 대각선은 자기 자신과 비교하기 때문에 항상 1
corr = pivot.corr()
...
for i in range(len(codes)):
    for j in range(i+1, len(codes)):
        ...
        if sector_of[codes[i]] == sector_of[codes[j]]:
            ...
            diff.append(v)
# 상관 행렬은 대칭이고, 대각선은 항상 1이다
    # j를 i+1부터 돌면 위쪽 삼각형만 훑게 되어 같은 쌍을 두 번 세거나、자기 자신을 섞이지 않고 추가할 수 있다
print(f"같은 섹터 쌍: {len(same)}개, 평균: {np.mean(same):.4f}")
print(f"다른 섹터 쌍: {len(diff)}개, 평균: {np.mean(diff):.4f}")
"""
    수치 상으로 보았을 때, 상관 관계가 섹터별로 존재한다 !
        > 그래프로는 판단하기 어렵다!
    시장 전체가 함께 움직이는 요인이 크다보니、
        섹터 차이가 심하게 발생하지는 않기 때문에 바로 식별할 정도가 되지는 않는다 !
"""
