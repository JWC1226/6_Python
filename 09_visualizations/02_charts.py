...
fig, axes = plt.subplots(1, 2, figsize=(13, 4))
    # ax.hist(값들, bins=구간수)
    # 값을 bins개의 구간으로 나누어서 각 구간에 몇 개의 데이터가 있는지 표시
    # hist는 NaN(결측)을 만나면 범위 계산이 깨진다 ! > dropna() 선행
...
# 박스플롯 = IQR 확인하기
    # IQR 표기 시 박스의 양 끝이 Q1, Q3 수염이 1.5 * IQR의 범위가 된다
    # 그 외의 점은 이상치로 표시가 된다
...
# 전체 데이터를 기준으로 이상치를 선별한다 > 사실 의미가 없다 !
    # >> 섹터별로 이상치를 선별해줘야 한다
...
# ns.barplot > 값을 그대로 그리지 않는다 !
    # 같은 sector에 속한 데이터를 평균을 내서 막대 하나로 표시한다
    # 합계를 표시하고자 할 땐 estimator="sum"을 지정해야 한다
    # 이미 groupby로 집계한 데이터를 넘기면 평균의 평균이 될 수 있다 !
