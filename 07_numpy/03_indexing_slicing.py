"""
    넘파이 배열의 인덱싱과 슬라이싱
    - 기본 인덱싱은 리스트와 동일
    - 슬라이싱은 리스트와 달리 복사본이 아니다
    　원본 배열의 뷰(View)를 반환한다 !
"""
import numpy as np
...
def normalize_wrong(arr):
    """
        [비정상 코드]
        : 마지막 5개의 데이터를 기준으로 최소값으로 값을 뺀 후、배열을 반환
    """
...
print(f"결과: {result}")
"""
          View (슬라이싱, ravel, reshape)      복사 (.copy, flatten)
    메모리 원본과 공유 (하나의 데이터)           새로운 메모리 공간에 복제 (독립된 두 개의 데이터)
    수정　 View를 수정하면 원본도 변경된다         복사본을 수정해도 원본은 그대로 유지된다
    속도　 빠르다 (복사 과정이 불필요)           상대적으로 느리다 (복사 과정이 필요)
"""