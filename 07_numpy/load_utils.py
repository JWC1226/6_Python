...
_COLUMNS = {
    "code": 0, "date": 1, "open": 2, "high": 3, "low": 4,
    "close": 5, "volume": 6, "change": 7, "changeRate": 8
}
...
def _read(col, dtype):
    """
        csv에서 하나의 열만 읽어서 1 차원 배열로 리턴해주는 함수
    """
    key = (col, str(dtype))
    if key not in _cache:
        ...
        _cache[key] = np.loadtxt(
            CSV_PATH, dtype=dtype,
            delimiter=",",          # 구분자
            usecols=_COLUMNS[col],  # 필요한 열 하나만 읽는다
            skiprows=1,             # 첫 줄을 생략 (건너뛴다)
            encoding="utf-8-sig"
        )
...
def load_close_flat():
    """
        종가 데이터만 1 차원 배열로 반환해준다
    """
    return _read("close", "int64")

def load_one_stock(idx = 0):
    """
        한 종목의 종가만 1 차원 배열로 리턴
        종목 하나 당 750개 (줄)、그 다음 항목… 750줄 ~
    """
...
def load_dates():
    """
        거래일 정보를 날짜 형식으로 반환 (1차원 배열)
        .astype("datetime64[D]") > 문자열을 날짜로 변경해준다
        [D] > Day 단위로 다루겠다는 의미
    """
...
def load_matrix():
    """
        종가 행렬을 반환
        헹: 종목 (120) / 열: 날짜 (750) >>> (120, 750)
    """
    close = load_close_flat()       # 1 차원 배열
    return close.reshape(120, 750)  # 2 차원 배열

def load_column(name):
    """
        열 데이터를 행렬(120, 750)로 반환해주는 함수
    """
...
_NAN_IDX = np.array([36, 42, 124, 720, 231, 365, 499, 512, 114, 237, 118, 702])
_OUTLIER_IDX = np.array([61, 215, 13, 488, 74])
_OUTLIER_SCALE = np.array([7.2, 5.4, 6.2, 3.6, 0.1])

def load_dirty():
    """
        결측, 이상치용 데이터
        첫 종목의 종가 데이터에 결측 12개、이상치 값 5개 추가
    """
