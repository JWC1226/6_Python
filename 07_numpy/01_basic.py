...
"""
import numpy as np
# Python 리스트에는 다양한 타입의 데이터를 담을 수 있다
data_list = [1, "이", 3.0, [4]]
...
# Python의 리스트는 데이터를 구분하지 않고 담을 수 있는 점이 편리할 수 있지만, 다른 리스트와 연산을 하고자 할 때는 문제가 될 수 있다!
# 넘파이 배열 (ndarray)은 데이터 타입이 하나로 고정된 구조이다 > 연산이 쉽다
...
# 넘파이 배열은 자바의 배열과 비슷하지만, 연산 수행 시 반복문을 사용하지 않고 배열 단위의 연산이 가능하다 (백터화 연산)
print("=" * 60)
...
print(f" - 배열 : {arr_bytes / 1024 / 1024:6.2f} MB")
    # 10만 개 기준으로 약 2~3MB 정도 차이가 나는 것을 확인