...
class CheckingAccount(Account):
    FEE = 500
    # 생성자를 정의하지 않을 것이다 !
    # Account(부모 타입) 생성자를 기준으로 생성할 수 있게 된다 !
    def withdraw(self, amount):
...
acc_list = [
    Account("이순신", 15970916),
    SavingsAccount("강감찬", 10190201),
    CheckingAccount("율곡 이이", 15361226)
]
for acc in acc_list:
    print(f"{acc.info()}")
print("=" * 45)
...
# Java: 공통 인터페이스 구현
# Python: 동일한 메소드 존재 여부만으로 실행이 가능하다
print("=" * 45)

# 다중 상속: 여러 부모 클래스를 상속할 수 있다
class Loggable:
    def log(self, message):
        return f"[LOG] {message}"
class Serializable:
    def to_dict(self):
        return self.__dict__            # 모든 필드를 딕셔너리로 변환시켜준다
