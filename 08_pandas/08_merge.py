...
"""
    시세(prices) 90,000행에 섹터 이름을 매번 저장하면 같은 문자열이 수 천번 반복될 것이다 !
        > 분리해서 저장하고 식별코드로 연결해준다 ! (정규화)
    저장할 때는 나누고, 분석할 때는 합쳐서 진행 !
        > 나누어져 있는 데이터를 다시 붙여주는 것 : merge
"""
# merge (SQL의 JOIN과 유사하다)
    # Left_df.merge(right_df, on="기준열", how="방식")
    # > 두 df의 기준열을 모두 가진 새로운 DataFrame 이 반환된다
...
# map(함수) : 대상 하나하나에 함수를 적용한 반환 값으로 새롭게 동일한 타입(시리즈)으로 반환
...
print(r1)
"""
    # 기준 열의 타입이 다르면 merge 시 오류가 발생한다 !
print("-" * 40) # --------------------------------------------------
