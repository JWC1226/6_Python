...
# agg : 요약표를 만들 때 그룹 별로 결과를 도출시킨다
# transform : 원본에 열을 추가해서 값을 비교하고자 할 때、그룹별로 계산 결과를 원본과 동일하게 도출시킨다
# filter : 그룹 별로 검사해서 조건에 맞지 않으면 제외된다

# 종목 코드가 "G0001", "G0002"인 데이터만 추출하여 two 변수에 저장한다
two = df[df["code"].isin(["G0001", "G0002"])].reset_index(drop=True)
print(two[["code", "date", "close"]].head(6))

# diff() : 바로 위 행과의 차이를 반환합니다.
    # s.diff() > s[i] - s[i-1]、맨 첫 행은 NaN
wrong = two["close"].diff()
right = two.groupby("code")["close"].diff()

# shift() : 열을 통째로 한 칸 아래로 밀어준다
    # s.shift() > 원본과 길이가 같은 Series를 반환한다
    # 1번째 값 = s[i-1]、맨 첫 항은 NaN
...
# filter : 그룹 단위로 걸러내준다・조건을 만족하는 그룹 전체를 날려준다 !
# 거래일이 700일 미만인 종목은 제외된다
...
print(f"> {len(filtered)} 행 {filtered['code'].nunique()} 종목")
    # 전 종목이 750일 데이터가 존재해서、걸러지는게 없다 !

# g["close"].mean() > 750일의 종목 평균
    # 종목 평균이 100,00 이하인 종목을 제외
big = df.groupby('code').filter(lambda g: g['close'].mean() > 100_000)
print(f"{len(big)} 행 {big['code'].nunique()} 종목")
"""
    filter 를 사용하면 특정 행이 아니라、그룹 전체를 남기거나 버린다 !
    df[df['close'] > 100_000] >> 조건에 맞는 행만 남긴다 !
"""
print("-" * 40) # --------------------------------------------------
...
print(multi.head(6).round(0))
    # 여러 개(n)의 열을 기준으로 그룹화를 하면, 인덱스가 n개인 시리즈로 반환된다 !
...
print(multi.unstack().head(4).round(0))
    # > 인덱스의 안쪽 레벨 값을 열로 올린다
print()

# reset_index() : 인덱스를 열로 되돌린다
print(multi.reset_index().head(3).round(0))
    # > 인덱스 값이 열이 되고, 인덱스는 0, 1, 2, … 으로 새로 만들어진다

# groupby(…, as_index=False) : 처음부터 그룹화 기준이 열로 나눈다
...
d['quarter'] = d['date'].dt.to_period("Q").astype(str)
    # to_period() : 날짜를 기잔으로 변경 (Q: 분기, M: 월, Y: 연)
...
wide = pv.iloc[:3, :3]
print("넓은 형식")
print(wide.round())     # 사람이 보기 편하다

# melt : 넓은 형식을 긴 형식으로 녹여서 표현한다
    # df.melt(id_vars=유지할_열, var_name=열-이름을_담을_열, value_name=값을_담을_열)
...
# DB 저장、시각화할 때 활용한다
# pivot > 사람이 보는 보고서, Excel, 데이터 등을을 확인 시 활용한다
