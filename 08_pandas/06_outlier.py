...
# raw-prices.csv 파일을 읽어와서
    # 'close': int64 / float64 > 콤마가 포함된 것도 변환된다
    # 'date': datetime64       > 형식이 섞여있음 / 모두 변환된다
# 'code', 'date' 열을 기준으로 중복 제거 (첫 번째 데이터를 남긴다)
# 위 결과를 df 변수에 저장한다
import pandas as pd
...
# 중복 제거
df = df.drop_duplicated(subset=['code', 'date'], keep='first').reset_index(drop=True)
print(len(df))
...
# 파일로 데이터를 저장 (.pkl)
    ...
    # 파일로부터 데이터 읽기 (.pkl)
        # read_pickle(경로) > 파일에 저장된 데이터를 데이터프레임으로 반환해 줄 것이다
print("파일로 저장")
...
print(f"""
        [종목 중앙값과 비교]
        1. 중앙값의 50배 초과: {(df['close'] > med * 50).sum()}건
        2. 중앙값의 5% 미만: {(df['close'] < med * 0.05).sum()}건
        >> 나름의 기준으로 너무 큰 값이나 너무 작은 값을 찾아볼 수 있다 !
""")
print("-" * 40) # --------------------------------------------------

# quantile() : 값을 크기순으로 나열했을 때、특정 위치의 값을 구해주는 함수
    # .quantile([0.25, 0.75])
    # > 값 2개짜리 시리즈가 반환된다 | Q1(25%), Q3(75%)
# IQR (사분위 범위) : Q3 - Q1
    # 규칙: Q1 - 1.5*IQR 보다 작은 경우、이상치
    # 　　  Q3 + 1.5*IQR 보다 큰 경우、이상치
    # >>> 평균, 표준편차와 달리 극단 값에 흔들리지 않기 때문에 이상치 탐지에 사용한다

q1, q3 = df['close'].quantile([0.25, 0.75])
print(f"q1: {q1}, q3: {q3}")
    # q1 > 상위 25% 1사분위 값 (12061.0)
    # q3 > 하위 25% 3사분위 값 (55106.25)
iqr = q3 - q1
lo = q1 - 1.5 * iqr
hi = q3 + 1.5 * iqr
...
# IQR로 이상치 탐지에 한계가 있다
    # 하한선이 음수가 나올 수가 없는데、규칙을 적용하니 음수값이 확인된다
# TODO: 중앙값 5% 미만
...
def is_outlier(data):
    """
        한 종목의 종가 데이터를 받아서 같은 길이의 bool mask(True/False)를 반환
        True : 이상치
        False : 정상 데이터
    """
...
print(f"증가 결측수: {df['close'].isna().sum()}")

print("_step2.pk1 저장")
df.to_pickle(step_path('_step2.pk1'))