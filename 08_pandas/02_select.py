...
one = last_history['close']     # one = 대괄호 1개로 감싸져있다
two = last_history[['close']]   # two = 대괄호 2개로 감싸져있다
print(f"last_history['close'] > {type(one).__name__} / shape: {one.shape}")
print(f"last_history[['close']] > {type(two).__name__} / shape: {two.shape}")
"""
    바깥 대괄호는 데이터프레임에서 조회(꺼내기)하기 위해서 사용한다
    안쪽 대괄호는 "목록"을 지정하기 위해 사용한다
"""
print("-" * 60) # --------------------------------------------------
...
print("종목 코드가 'G0001', 'G0050', 'G0100'만 조회")
result = df[df['code'].isin(['G0001', 'G0050', 'G0100'])]
print(result)
print("-" * 80) # --------------------------------------------------

"""
    loc vs iloc : 행/열을 선택할 때 (Indexing) 및 범위를 선택할 때 (Slicing)
    ...
    - iloc (Integer Location) : 정수(위치) 기반 선택
      ...
      - 행 선택: 0부터 시작하는 위치 (숫자)
      - 열 선택: 0부터 시작하는 위치 (숫자)
      　⇒ 위치 기반으로 기존 Slicing 과 동일 / "끝 위치" 미포함 (제외)
"""
print("loc")
print(df.loc[0:2])      # 3건
...
# TODO: 'G0001' 종목의 'close', 'changeRate' 만 조회
