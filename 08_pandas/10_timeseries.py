...
# 날짜 데이터를 인덱스로 사용한다
    ...
    # one.info()
one = one.set_index('data').sort_index()
...
# 날짜 인덱스로 슬라이싱 할 때는 이전에 정렬이 되어있어야한다 ! (sort_index())
print("-" * 40) # --------------------------------------------------

# dt 접근자
    # 날짜 함수, 연/월/일 같은 조각을 꺼낼 때 사용한다
d = df.head(3)
...
# resample : 시간 단위로 바꿔서 다시 묶어준다
    # df.resample(시간단위).집계함수
    # > 시간 단위만큼의 행을 가진 결과・뒤에 추가한 집계함수가 값을 정해준다
        # >> 시간 기준으로 groupby
...
# resample("ME").ohlc()
    ...
    # > 기간별 시가, 고가, 저가, 종가 네개열을 한 번에 만들 수 있다
print(one['close'].resample('ME').ohlc().head(3).round(0))
print("-" * 40) # --------------------------------------------------

# rolling : 연속된 N개의 행을 윈도우 단위로 훑으면서 계산
    # 이동 평균처럼 최근 N일의 데이터가 필요할 때 사용한다
...
"""
    groupby 없이 계산하면 {wrong} 종목별 코드가 달라도 이평선 재계산없이 이어지는 형태도 계산이 된다
    그룹화를 해줘야 종목별 20일 이평선이 정상적으로 계산된다
    diff, shift, rolling > 모두 groupby 가 필요하다 !
"""
print("-" * 40) # --------------------------------------------------
# 변화율 계산
...
# 누적 수익률
    # > 첫 날의 NaN을 0으로 채우고、(1+수익률)을 차례로 곱한다
